...
def call_api(
    ...
    Ollama does not support n>1 in a single request, so we issue n_samples
    sequential calls and aggregate the choices ourselves.
    temperature=0.8, max_tokens=1024.
    Pass extra_body={"enable_thinking": False} to disable Qwen3.5 thinking mode.

    Returns list of Choice objects.
    """
    ...
    for _ in range(n_samples):
        try:
            ...
            # One sample failed — log and continue collecting the rest
            print(f"  API call failed (skipping sample): {e}", file=sys.stderr)
    return choices
