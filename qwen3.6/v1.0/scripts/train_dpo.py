"""
Qwen3.6-DeltaCoder-9B — DPO training script (Qwen3.6 v1.0).
...
Usage:
    ...
    python scripts/train_dpo.py --sft-model /workspace/merged_qwen3.6-v1.0 --max-steps 50

    # Full training:
    python scripts/train_dpo.py --sft-model /workspace/merged_qwen3.6-v1.0

    # Full training + Ling-Coder-DPO mix (253K coding preference pairs):
    python scripts/train_dpo.py --sft-model /workspace/merged_qwen3.6-v1.0 --ling-coder 50000
"""
...
BASE_MODEL_NAME = "Qwen/Qwen3.6-9B"
MAX_SEQ_LENGTH = 4096
LORA_R = 32
LORA_ALPHA = 32

# TODO: Verify LoRA target modules match Qwen3.6 architecture (GDN may differ)
LORA_TARGET_MODULES = [
    # Full Attention
    "q_proj",
    "k_proj",
    "v_proj",
    "o_proj",
    # GDN (Gated DeltaNet) — Qwen3.5-specific, 24/32 layers. May differ in 3.6.
    "in_proj_qkv",
    "in_proj_z",
    "in_proj_b",
    "in_proj_a",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="DPO training for DeltaCoder Qwen3.6 v1.0"
    )
    parser.add_argument(
        "--sft-model",
        type=str,
    ...
    parser.add_argument(
        "--data",
        type=str,
        default="data/dpo_pairs_v1.0.jsonl",
        help="Path to DPO pairs JSONL in conversational format",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./outputs/deltacoder-9b-qwen3.6-v1.0-dpo",
...
def main():
    ...
    dpo_config_kwargs = dict(
        ...
        dataset_num_proc=1,  # Qwen tokenizer may crash with multiprocessing
