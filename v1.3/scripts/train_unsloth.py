...
# Qwen3.6 sizes TBD — if no 9B, may need to adjust (e.g. Qwen/Qwen3.6-8B or -14B)
...
# If architecture changes, inspect model.named_modules() to find correct projection names.
LORA_TARGET_MODULES = [
    # Full Attention (8 of 32 layers in Qwen3.5 — may differ in 3.6)
    "q_proj",
    "k_proj",
    "v_proj",
    "o_proj",
    # GDN / Gated Delta Net (24 of 32 layers in Qwen3.5 — may differ in 3.6)
    "in_proj_qkv",
    "in_proj_z",
    "in_proj_b",
    "in_proj_a",
...
def main():
    ...
    if is_pretokenized:
        ...
        # Cache the tokenized dataset on disk so we don't re-tokenize all rows every run
    ...
    sft_config_kwargs = dict(
        ...
        tf32=True,
        seed=42,
        dataset_num_proc=1,  # CRITICAL: Qwen tokenizer crashes with multiprocessing
