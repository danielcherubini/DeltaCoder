"""
Pre-tokenize DeltaCoder v1.3 training data.

KEY CHANGE vs v1.2: SEQUENCE_LEN = 32768 (was 8192).
Captures full OCR reasoning traces — nvidia/OpenCodeReasoning averages 8,477 tokens
with 50% exceeding 8192. NVIDIA trained at 32K context. v1.2 truncated half the traces.

Bypasses Axolotl's O(n²) chat_template bug.
...
def main():
    input_path = sys.argv[1] if len(sys.argv) > 1 else "data/v1.2_sft_train.jsonl"
    output_path = (
        sys.argv[2] if len(sys.argv) > 2 else "/dev/shm/train_tokenized_v1.3.jsonl"
    )
